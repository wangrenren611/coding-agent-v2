...
const parseToolResultFromData = (value: unknown): ParsedToolResult | null => {
  ...
  const status =
    successValue === true ? "success" : successValue === false ? "error" : "unknown";
...
const resolveStructuredResultObject = (
  ...
  return (
    readObject(result?.payload) ??
    readObject(result?.metadata) ??
    parseJsonObject(result?.details)
  );
};
...
const summarizeAgentRun = (run: Record<string, unknown>, extras?: Record<string, unknown>): { lines: string[]; output?: string } => {
  ...
  const meta = formatSummaryMeta([
    ...
    readNumber(extras?.waited_ms) !== undefined ? `${Math.round((readNumber(extras?.waited_ms) ?? 0) / 1000)}s waited` : null,
  ]);
  if (meta) {
    lines.push(meta);
  }
...
const buildTaskHeaderDetail = (toolName: string, args: Record<string, unknown> | null): string | null => {
  if (!args) {
    return null;
  }
  ...
  if (toolName === "task_create") {
    ...
    return formatSummaryMeta([
      ...
      readArray(args.checkpoints).length > 0 ? `${readArray(args.checkpoints).length} checkpoints` : null,
    ]);
  }
  ...
  if (toolName === "task_list") {
    const statuses = readArray(args.statuses).map((value) => readString(value)).filter(Boolean).join(", ");
    return formatSummaryMeta([
  ...
  if (toolName === "task_update") {
    ...
    if (readString(args.status)) changes.push(`status -> ${readString(args.status)?.replace(/_/g, " ")}`);
    if (readNumber(args.progress) !== undefined) changes.push(`progress ${Math.round(readNumber(args.progress) ?? 0)}%`);
    if (readString(args.owner)) changes.push(`owner ${readString(args.owner)}`);
    if (readArray(args.add_blocked_by).length > 0) changes.push(`+${readArray(args.add_blocked_by).length} blockers`);
    if (readArray(args.remove_blocked_by).length > 0) changes.push(`-${readArray(args.remove_blocked_by).length} blockers`);
    return changes.join(" · ");
  }
...
const buildTaskResultSections = (
  ...
  if (toolName === "task_list") {
    ...
    const tasks = readArray(payload.tasks).map((item) => readObject(item)).filter((item): item is Record<string, unknown> => Boolean(item));
  ...
  if (toolName === "task_stop") {
    ...
    const cancelledTaskIds = readArray(payload.cancelled_task_ids).map((item) => readString(item)).filter(Boolean);
...
const buildSearchHeaderDetail = (toolName: string, args: Record<string, unknown> | null): string | null => {
  if (!args) {
    return null;
  }
  ...
  if (toolName === "grep") {
    ...
    return formatSummaryMeta([
      ...
      readNumber(args.max_results) !== undefined ? `limit ${Math.round(readNumber(args.max_results) ?? 0)}` : null,
      readNumber(args.timeout_ms) !== undefined ? `${Math.round((readNumber(args.timeout_ms) ?? 0) / 1000)}s timeout` : null,
    ]);
  }
...
export const AssistantToolGroup = ({ group }: AssistantToolGroupProps) => {
  ...
  const hasInvocationDetails = Boolean(invocationDetails);
    ...
    <box flexDirection="column" marginTop={1}>
      <box paddingLeft={3}>
        <text fg={uiTheme.text} attributes={uiTheme.typography.note} wrapMode="word">
          <span fg={uiTheme.accent}>{icon}</span> {specialPresentation?.toolLabel ?? formatToolName(toolName)}
