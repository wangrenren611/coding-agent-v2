...
import type { AgentToolResultEvent, AgentToolStreamEvent, AgentToolUseEvent } from "../agent/runtime/types";
...
describe("buildAgentEventHandlers", () => {
  it("keeps ordered stream segments as thinking -> text -> thinking -> tool -> tool result", () => {
    ...
    const toolResultIndex = segments.findIndex((segment) => segment.id === `${turnId}:tool-result:call_1`);
  ...
  it("deduplicates repeated tool-use events for the same toolCallId", () => {
    ...
    const toolUseSegments = readSegments().filter((segment) => segment.id === `${turnId}:tool-use:call_1`);
