...
import { SqliteTaskRunner } from '../task-v2/runtime/sqlite-runner';
import { TaskService } from '../task-v2/service';
import { SqliteTaskRepository } from '../task-v2/storage/sqlite-repository';
...
describe('task-v2 core (repository/service/runner)', () => {
  let tempDir: string;
  let dbPath: string;
  ...
  beforeEach(async () => {
    tempDir = await fsp.mkdtemp(path.join(os.tmpdir(), 'task-v2-core-'));
