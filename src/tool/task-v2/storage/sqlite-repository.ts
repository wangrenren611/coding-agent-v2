...
const TASK_V2_DDL = `
  CREATE TABLE IF NOT EXISTS task_v2_tasks (
  ...
  CREATE INDEX IF NOT EXISTS idx_task_v2_tasks_session_status
    ON task_v2_tasks(session_id, status, updated_at DESC);
  CREATE INDEX IF NOT EXISTS idx_task_v2_tasks_session_priority
    ON task_v2_tasks(session_id, priority, updated_at DESC);

  CREATE TABLE IF NOT EXISTS task_v2_dependencies (
    session_id TEXT NOT NULL,
    task_id TEXT NOT NULL,
    depends_on_task_id TEXT NOT NULL,
    created_at TEXT NOT NULL,
    PRIMARY KEY (session_id, task_id, depends_on_task_id),
    FOREIGN KEY (session_id, task_id) REFERENCES task_v2_tasks(session_id, id) ON DELETE CASCADE,
    FOREIGN KEY (session_id, depends_on_task_id) REFERENCES task_v2_tasks(session_id, id) ON DELETE CASCADE
  );

  CREATE INDEX IF NOT EXISTS idx_task_v2_dependencies_depends_on
    ON task_v2_dependencies(session_id, depends_on_task_id);

  CREATE TABLE IF NOT EXISTS task_v2_runs (
    session_id TEXT NOT NULL,
    id TEXT NOT NULL,
    task_id TEXT,
    ...
    updated_at TEXT NOT NULL,
    PRIMARY KEY (session_id, id),
    FOREIGN KEY (session_id, task_id) REFERENCES task_v2_tasks(session_id, id) ON DELETE SET NULL
  );

  CREATE INDEX IF NOT EXISTS idx_task_v2_runs_session_status
    ON task_v2_runs(session_id, status, updated_at DESC);
  CREATE INDEX IF NOT EXISTS idx_task_v2_runs_status
    ON task_v2_runs(status, updated_at DESC);
  CREATE INDEX IF NOT EXISTS idx_task_v2_runs_finished_at
    ON task_v2_runs(finished_at);

  CREATE TABLE IF NOT EXISTS task_v2_run_events (
    session_id TEXT NOT NULL,
    ...
    FOREIGN KEY (session_id, run_id) REFERENCES task_v2_runs(session_id, id) ON DELETE CASCADE
  );

  CREATE INDEX IF NOT EXISTS idx_task_v2_run_events_cursor
    ON task_v2_run_events(session_id, run_id, seq);
`;
...
export class SqliteTaskRepository implements TaskRepository {
  ...
  private async doPrepare(): Promise<void> {
    ...
    await this.client.exec(TASK_V2_DDL);
    await this.ensureRunColumns();
    this.prepared = true;
  }

  private async ensureRunColumns(): Promise<void> {
    const columns = await this.client.all<TableInfoRow>('PRAGMA table_info(task_v2_runs)');
    const names = new Set(columns.map((column) => column.name));
    if (!names.has('agent_profile_id')) {
      await this.client.exec('ALTER TABLE task_v2_runs ADD COLUMN agent_profile_id TEXT;');
    }
    if (!names.has('agent_config_snapshot')) {
      await this.client.exec('ALTER TABLE task_v2_runs ADD COLUMN agent_config_snapshot TEXT;');
    }
  }
  ...
  async deleteTask(sessionId: string, taskId: TaskId): Promise<boolean> {
    ...
    await this.client.run('DELETE FROM task_v2_tasks WHERE session_id = ? AND id = ?', [
      sessionId,
      taskId,
    ]);
    return true;
  ...
  async addDependency(sessionId: string, edge: TaskDependency): Promise<void> {
    ...
    await this.client.run(
      `
        INSERT OR IGNORE INTO task_v2_dependencies(
  ...
  async removeDependency(
    ...
    await this.client.run(
      `
        DELETE FROM task_v2_dependencies
  ...
  async appendRunEvent(sessionId: string, input: AppendRunEventInput): Promise<RunEvent> {
    ...
    return this.client.transaction(async () => {
      ...
      await this.client.run(
        `
          INSERT INTO task_v2_run_events(
  ...
  async clearSession(sessionId: string): Promise<void> {
    ...
    await this.client.transaction(async () => {
      await this.client.run('DELETE FROM task_v2_tasks WHERE session_id = ?', [sessionId]);
      await this.client.run('DELETE FROM task_v2_runs WHERE session_id = ?', [sessionId]);
    });
  }
  ...
  async gcRuns(params: { finishedBefore: string; limit: number }): Promise<number> {
    ...
    return this.client.transaction(async () => {
      ...
      for (const row of rows) {
        await this.client.run('DELETE FROM task_v2_runs WHERE session_id = ? AND id = ?', [
          row.session_id,
          row.id,
        ]);
      }
