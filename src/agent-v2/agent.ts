...
export class Agent {
    ...
    async run(query: string, options?: LLMGenerateOptions) {
        ...
        try {
            ...
            this.hookManager.executeStopHooks(
                { reason: this.state.aborted ? 'aborted' : this.state.resultStatus, message: this.state.lastError?.message },
                this.getHookContext()
            );
        }
    }
    ...
    private shouldStop(): boolean {
        ...
        if (lastMessage && (MessageGuard.isToolCall(lastMessage) || MessageGuard.isToolResult(lastMessage))) {
            return false;
        }
    ...
    private async executeStep(options?: LLMGenerateOptions): Promise<{
        ...
        // 4. 处理响应流
        // ========================================
        const { finishReason, toolCalls, text, reasoningContent } = await this.processResponse(response);
    ...
    private mergeToolCalls(existing: ToolCall[], newCalls: ToolCall[]): ToolCall[] {
        ...
        for (const newCall of newCalls) {
            ...
            if (existingIndex >= 0) {
                ...
                const existingArgs = typeof existingCall.arguments === 'string' 
                    ? existingCall.arguments 
                    : JSON.stringify(existingCall.arguments || {});
                const newArgs = typeof newCall.arguments === 'string' 
                    ? newCall.arguments 
    ...
    private async processToolCalls(toolCalls: ToolCall[]): Promise<void> {
        ...
        for (const toolCall of toolCalls) {
            ...
            // ========================================
            const modifiedToolCall = await this.hookManager.executeToolUseHooks(toolCall, this.getHookContext());
