...
describe('HookManager', () => {
  ...
  describe('Step Hook', () => {
    it('应该正确执行 step hooks', async () => {
      ...
      hookManager.use(plugin);
      await hookManager.executeStepHooks(
        { stepIndex: 1, finishReason: 'stop', toolCallsCount: 0 },
        mockContext
      );
  ...
  describe('LLM Config Hook', () => {
    ...
    it('应该链式执行多个 llmConfig hooks', async () => {
      const plugins: Plugin[] = [
        ...
        {
          name: 'second',
          llmConfig: async (config) => ({ ...config, temperature: config.temperature! + 0.1 }),
        },
      ];
