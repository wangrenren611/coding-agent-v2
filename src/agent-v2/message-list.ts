...
export class MessageList {
    ...
    getToolResultForCall(toolCallId: string): ToolResultMessage | undefined {
        return this._messages.find(
            (m): m is ToolResultMessage => 
                MessageGuard.isToolResult(m) && m.tool_call_id === toolCallId
        );
    }
     ...
     */
    updateMessageById(messageId: string, updates: Partial<{
        content: string;
        reasoning_content: string;
        tool_calls: ToolCall[];
        finish_reason: string;
     ...
     */
    async updateMessageByIdAsync(messageId: string, updates: Partial<{
        content: string;
        reasoning_content: string;
        tool_calls: ToolCall[];
        finish_reason: string;
     ...
     * 异步更新最后一条消息（带锁）
     */
    async updateLastMessageAsync(updates: Partial<{
        content: string;
        reasoning_content: string;
        tool_calls: ToolCall[];
        finish_reason: string;
